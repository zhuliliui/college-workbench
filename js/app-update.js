...
(function () {
  ...
  function http() { const C = cap(); return C && C.Plugins && C.Plugins.Http ? C.Plugins.Http : null; }
  ...
  async function check() {
    if (!isNative()) return;
    const U = updater();
    if (!U) { console.warn('CapacitorUpdater 未安装，跳过热更新'); return; }
    ...
    if (manifest.version === installed) return; // 已是最新
    if (!manifest.url) return;

    if (window.UI) UI.toast('发现新版本，正在后台更新…', 'ok');
    try {
      ...
      if (window.UI) UI.toast('更新失败，下次启动再试', 'warn');
    }
  }
