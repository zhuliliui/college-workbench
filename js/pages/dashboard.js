...
Pages.dashboard = function () {
  ...
  let taskListHtml;
  if (todayTotal === 0) {
    taskListHtml = `<div class="empty soft"><span class="emoji">📚</span>
    ...
    taskListHtml = '<div class="list home-list">' + items.map((t) => {
      return `<div class="item ${t.done ? 'done' : ''}" data-id="${t.id}">
        <button class="check" data-act="toggle-task" data-id="${t.id}" aria-label="完成">${t.done ? '✓' : ''}</button>
  ...
  if (upcomingDDLs.length === 0) {
    ddlListHtml = `<div class="empty soft"><span class="emoji">⏰</span>
  ...
  // 快速入口
  const quicks = [
    { id: 'study', emoji: '📖', label: '学习复习' },
    { id: 'ddl', emoji: '⏰', label: 'DDL倒计时' },
    { id: 'finance', emoji: '💰', label: '记账存钱' },
    { id: 'discipline', emoji: '⭐', label: '自律成长' },
    { id: 'travel', emoji: '🏖', label: '旅行规划' },
    { id: 'review', emoji: '📅', label: '月度复盘' },
    { id: 'english', emoji: '📚', label: '考研英语' },
  ];
  ...
  <div class="welcome-banner">
    <div class="welcome-text">
      ...
      <div class="welcome-sub">这里是你的专属学习生活中枢，所有进度自动同步到虚拟存钱罐 🐷</div>
    </div>
  </div>
  ...
  <div class="grid grid-2">
    <div class="card">
      <div class="card-head">
        <div class="title"><span class="ic">📖</span>今日学习任务 <span class="sub" style="margin-left:8px">${todayDone}/${todayTotal} 已完成</span></div>
    ...
    <div class="card">
      <div class="card-head">
        <div class="title"><span class="ic">⏰</span>临近 DDL <span class="sub" style="margin-left:8px">${upcomingDDLs.length} 项</span></div>
        <div class="spacer"></div>
  ...
  <div class="card">
    <div class="card-head">
      <div class="title"><span class="ic">🚀</span>快速进入</div>
    </div>
    <div class="card-body">
