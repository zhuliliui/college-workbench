...
Pages.discipline = function () {
  ...
  const itemsHtml = items.length ? '<div class="grid grid-2">' + items.map((it) => {
    ...
    return `<div class="card" style="margin:0;padding:14px">
      <div class="flex-between">
        <div style="font-weight:700;font-size:15px">${it.icon || '✨'} ${UI.esc(it.name)}</div>
        <div class="ops">
          <button class="btn btn-soft btn-icon" data-act="item-edit" data-id="${it.id}" title="编辑">✏️</button>
          <button class="btn btn-soft btn-icon" data-act="item-del" data-id="${it.id}" title="删除">🗑</button>
        </div>
      </div>
    ...
    : `<div class="empty"><span class="emoji">⭐</span><div class="t">还没有打卡项目</div><div class="s">添加作息、运动、阅读、技能练习等，每天打卡攒奖励</div></div>`;
  ...
  const statHtml = items.length ? items.map((it) => {
    ...
    return `<div style="margin-bottom:12px">
      <div class="flex-between"><div class="name" style="font-weight:600">${it.icon || '✨'} ${UI.esc(it.name)}</div>
        <div class="muted-text">${cnt}/${elapsed} 天 · ${pct}%</div></div>
      <div class="progress mt8"><span style="width:${pct}%"></span></div>
    </div>`;
  }).join('') : `<div class="empty"><span class="emoji">📊</span><div class="t">暂无打卡数据</div></div>`;

  c.innerHTML = `
  <div class="card">
    <div class="card-head"><div class="title"><span class="ic">⭐</span>自定义打卡项目</div>
  ...
  <div class="card">
    <div class="card-head"><div class="title"><span class="ic">📝</span>每日自律打分 · ${today}</div>
  ...
  <div class="card">
    <div class="card-head"><div class="title"><span class="ic">📈</span>月度打卡统计</div>
  ...
  function openItem(editId) {
    ...
    UI.openModal({ title: it ? '编辑打卡项' : '添加打卡项', icon: '✨',
      body: `
      <div class="row">
